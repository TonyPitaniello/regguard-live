    ...
    <div className="min-h-screen bg-slate-950 text-white">
      <div className="max-w-3xl mx-auto px-4 py-10 sm:px-6 space-y-8">
        <header className="space-y-3 border-b border-slate-800 pb-6">
          <p className="text-xs font-bold uppercase tracking-wide text-purple-300">RegGuard shareable report</p>
          <h1 className="text-3xl font-black">Site diligence punch list</h1>
          <p className="text-gray-300">
            {analysis.project_info.address}
            <br />
            {analysis.project_info.city}, {analysis.project_info.state} {analysis.project_info.zip}
          </p>
          <div className="flex flex-wrap gap-2 pt-2">
            <button
              type="button"
              onClick={() => void copyLink()}
              className="inline-flex items-center gap-2 rounded-lg border border-purple-500/40 bg-purple-500/10 px-3 py-2 text-sm font-semibold text-purple-100 hover:bg-purple-500/20"
            >
        ...
        <section className="grid sm:grid-cols-3 gap-3">
          <div className="rounded-lg border border-slate-700 bg-slate-900/60 p-4">
            ...
            <p className="text-xl font-black text-amber-200">{hideRisk ? 'Unavailable' : analysis.environmental_screening.risk_level}</p>
          </div>
          <div className="rounded-lg border border-slate-700 bg-slate-900/60 p-4">
        ...
        <section className="space-y-3">
          ...
          {sources.length === 0 ? (
            <p className="text-sm text-gray-400">
              No citeable .gov / Municode URLs in this preview. Full research runs attach sources when scout hits exist.
            </p>
          ) : (
              ...
              {sources.map((s, i) => (
                <li key={i} className="text-gray-300">
                  {s.url?.startsWith('http') ? (
                    <a href={s.url} target="_blank" rel="noopener noreferrer" className="text-purple-300 hover:underline">
                      {s.label || s.url}
                    </a>
                  ) : (
                    s.label
        ...
        <footer className="border-t border-slate-800 pt-6 text-sm text-gray-400">
          Built for forwarding into a bid file. Confirm every fee and timeline with the AHJ before you bid.
          <div className="mt-3">
            <Link to="/" className="text-purple-300 hover:text-purple-200 font-semibold">
              Run your own RegGuard lookup →
            </Link>
          </div>
        </footer>
      </div>
